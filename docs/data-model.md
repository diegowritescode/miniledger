...
The **`outbox`** table records domain events **in the same transaction as the ledger write**, so an
event can never disagree with whether the money committed ([ADR-010](adr/010-transactional-outbox.md)).
A later relay (the EventBridge spine project) publishes unpublished rows and stamps `published_at`.

### `outbox`

| Column         | Type          | Null | Default             | Notes                                                       |
| -------------- | ------------- | ---- | ------------------- | ----------------------------------------------------------- |
| `id`           | `uuid`        | no   | `gen_random_uuid()` | Primary key; the event id consumers dedupe on.              |
| `type`         | `text`        | no   | —                   | Event name, e.g. `transfer.posted`.                         |
| `payload`      | `jsonb`       | no   | —                   | The event body (e.g. the transfer receipt).                 |
| `created_at`   | `timestamptz` | no   | `now()`             | When the event was recorded.                                |
| `published_at` | `timestamptz` | yes  | —                   | `NULL` until relayed; the relay is deferred to EventBridge. |
